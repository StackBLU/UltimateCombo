			...
			if (!GetResetValues(config))
			{
				...
				foreach (int value in values)
				{
					...
					if (presets.Contains(value))
					{
						...
						{
							Service.ChatGui.PrintError($"[StackCombo] Some features have been disabled due to an internal configuration update:");
							needToResetMessagePrinted = !needToResetMessagePrinted;
						}

						Attributes.CustomComboInfoAttribute? info = preset.GetComboAttribute();
						Service.ChatGui.PrintError($"[StackCombo] - {info.JobName}: {info.FancyName}");
				...
				if (needToResetMessagePrinted)
				{
					Service.ChatGui.PrintError($"[StackCombo] Please re-enable these features to use them again. We apologise for the inconvenience");
				}
			}
