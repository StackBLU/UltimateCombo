...
namespace UltimateCombo.Combos.PvE
{
	internal static class BLU
	{
				...
				if (actionID is MoonFlute && IsEnabled(CustomComboPreset.BLU_MoonFluteOpener))
				{
					...
					if (IsNotEnabled(CustomComboPreset.BLU_MoonFluteOpener_DoTOpener) && actionID is MoonFlute)
					{
						...
						if (IsOffCooldown(WingedReprobation) && actionID is MoonFlute && IsSpellActive(WingedReprobation) && WasLastAbility(SeaShanty)
							&& SafeFindEffect(Buffs.WingedReprobation).Param == 1)
						{
							return WingedReprobation;
						}
		...
		internal class BLU_Treasure_Healer : CustomComboClass
		{
			protected internal override CustomComboPreset Preset { get; } = CustomComboPreset.BLU_Treasure_Healer;
			...
			protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
			{
					...
					&& !HasEffect(Buffs.PhantomFlurry) && GetPartyMembers().Length == 0)
				{
					if (notInMap)
					{
						if (IsEnabled(CustomComboPreset.BLU_Treasure_Healer_MightyGuard) && HasEffect(Buffs.MightyGuard))
						{
							return MightyGuard;
						}
		...
		internal class BLU_Treasure_Tank : CustomComboClass
		{
			protected internal override CustomComboPreset Preset { get; } = CustomComboPreset.BLU_Treasure_Tank;
			...
			protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
			{
					...
					&& !HasEffect(Buffs.PhantomFlurry) && GetPartyMembers().Length == 0)
				{
					if (notInMap)
					{
						if (IsEnabled(CustomComboPreset.BLU_Treasure_Tank_MightyGuard) && HasEffect(Buffs.MightyGuard))
						{
							return MightyGuard;
						}
